...
@section('title', '従業員編集')
@section('content')
<h3 class="my-3">従業員編集</h3>
<form method="post" action="{{ route('admin.member.update', $admin->id) }}" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
    <div class="col-sm-6 mt-4 mb-3">
        <label for="name" class="mb-1"><span class="badge bg-danger">必須</span>  氏名</label>
        <input class="form-control mb-2" id="name" type="text" name="name" value="{{ old('name', $admin->name) }}">
        @error('name')
    ...
    <div class="col-sm-6 mb-3">
        ...
        <input class="form-control mb-2" id="jan" type="text" name="jan" value="{{ old('jan', $admin->jan) }}">
        @error('jan')
            <li class="list-unstyled text-danger">{{ $message }}</li>
        @enderror
    </div>
    ...
    <div class="col-sm-6 mb-4">
        ...
        <select class="form-select" id="role" name="role">
            ...
            @foreach ($roles as $role)
                <option value="{{ $role->id }}" @if($role->id == old('role', $admin->role_id)) selected @endif>{{ $role->position }}</option>
            @endforeach
        </select>
        @error('role')
            <li class="list-unstyled text-danger">{{ $message }}</li>
