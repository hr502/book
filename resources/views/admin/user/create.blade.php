...
@section('content')
    ...
    <form method="post" action="{{ route('admin.user.store') }}">
        ...
        <div class="col-sm-6 mb-3">
            ...
            <input class="form-control mb-2" id="name_kana" type="text" name="name_kana" value="{{ old('name_kana') }}">
            @error('name_kana')
                <li class="list-unstyled text-danger">{{ $message }}</li>
            @enderror
        </div>
        ...
        <div class="col-sm-6 mb-3">
            ...
            <input class="form-control mb-2" id="email" type="text" name="email" value="{{ old('email') }}">
            @error('email')
                <li class="list-unstyled text-danger">{{ $message }}</li>
            @enderror
        </div>
        ...
        <button class="btn btn-secondary mb-3" type="button"
            onclick="location.href='{{ route('admin.user.index') }}'">キャンセル</button>
